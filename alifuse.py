#!/usr/bin/env python
"""
A FUSE wrapper for locally mounting Aliyun OSS fs
"""
...
class AliyunFs(LoggingMixIn, Operations):
    ...
    def __init__(self, account, key):
        self.blobs = OssAPI(HOST, account, key)
    ...
    def rebuild_container_list(self):
        cmap = dict()
        cnames = set()
        res = self.blobs.get_service()
        if (res.status / 100) == 2:
            body = res.read()
            buckets = GetServiceXml(body)
    ...
    def _get_dir(self, path, contents_required=False):
        ...
        if '/' + cname not in self.containers:
            ...
            if self.containers['/' + cname]['files'] is None:
                ...
                res = self.blobs.get_bucket(cname, "", "", "", "", {})
                if (res.status / 100) == 2:
                    body = res.read()
                    h = GetBucketXml(body)
    ...
    def mkdir(self, path, mode):
        if path.count('/') <= 1:    # create on root
            ...
            resp = self.blobs.create_bucket(name)
    ...
    def rmdir(self, path):
        if path.count('/') == 1:
            c_name = path[1:]
            resp = self.blobs.delete_bucket(c_name)
    ...
    def open(self, path, flags=0, data=None):
        if data == None:                    # download contents
            ...
            data = self.blobs.get_object(c_name, f_name, {}).read()
        self.fd += 1
    ...
    def flush(self, path, fh=None):
        if not fh:
            ...
            self.blobs.put_object_with_data(c_name, f, data, 'BlockBlob')
            # else:
            #     self.blobs.upload_large_file()
    ...
    def unlink(self, path):
        ...
        self.blobs.delete_object(c_name, f)
...
if __name__ == '__main__':
    if len(argv) < 4:
        ...
        exit(1)
    fuse = FUSE(AliyunFs(argv[2], argv[3]), argv[1], debug=False,
